...
document.addEventListener('keyup', function (type) {
...
function isPossible() {
  ...
  for (var row = 0; row < rows; row++) {
    for (var col = 0; col < columns; col++) {
      if (row < 3) {
        if (gameField[row][col] === gameField[row + 1][col] || gameField[row][col] === gameField[row][col + 1]) {
          check = true;
        }
      } else {
...
function slideUp() {
  for (var column = 0; column < columns; column++) {
    var row = [gameField[0][column], gameField[1][column], gameField[2][column], gameField[3][column]];
    row = slide(row);
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "1154" + '/');
